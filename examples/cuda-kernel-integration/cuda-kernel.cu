...
__global__ __launch_bounds__(default_block_size) void parsinv_kernel(
    int n,     // matrix size
    int Lnnz,  // number of nonzeros in LT stored in CSR, upper triangular
               // (equivalent to L in CSC)
    ...
    const int* __restrict__ sym_map)
{
    ...
    if (threadidx < Snnz) {
        ...
        if (i > j) {
            return;
            // alternative: swap indices to compute (i,j) using the formula for
            // (j,i) int t = i; i = j; j = t;
        }
...
{
    unsigned int grid_dim =
        (Annz + default_block_size - 1) / default_block_size;

    parsinv_residual_kernel<<<dim3(grid_dim), dim3(default_block_size)>>>(
        n, Annz, Arowptr, Arowidx, Acolidx, Aval, Srowptr, Scolidx, Sval, tval);
}


// computes spones(A).*B and writes result in A
__global__ void ASpOnesB_kernel(int n,               // matrix size
                                int* Arowptr,        // row pointer A
                                int* Acolidx,        // col index A
                                double* Aval,        // val array A
...
)
{
    ...
    ASpOnesB_kernel<<<dim3(grid_dim), dim3(default_block_size)>>>(
        n, Arowptr, Acolidx, Aval, Browptr, Bcolidx, Bval);
}
